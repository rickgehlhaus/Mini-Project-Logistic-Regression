...
sub_nh11 <- subset(NH11, select = c("everwrk", "age_p", "r_maritl"))
NH11_t <- transform(NH11, everwrk = factor(everwrk, levels = c("1 Yes", "2 No")), r_maritl = droplevels(r_maritl))
mod.work <- glm(everwrk ~ age_p + r_maritl, data = NH11_t, family = "binomial")
summary(mod.work)
##   2. Predict the probability of working for each level of marital
##      status.
library(effects)
data.frame(Effect("r_maritl", mod.work))
